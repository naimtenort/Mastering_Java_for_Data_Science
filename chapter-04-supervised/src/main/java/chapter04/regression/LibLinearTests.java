...
public class LibLinearTests {
    ...
    public static void main(String[] args) throws IOException {
        chapter04.classification.LibLinearTests.mute();
        ...
        for (double eps : es) {
            for (double C : Cs) {
                DescriptiveStatistics summary = crossValidate(folds, fold -> {
                    ...
                    return chapter04.classification.LibLinearTests.train(fold, param);
                });
    ...
    private static double rmse(Model model, Dataset dataset) {
        double[] prediction = chapter04.classification.LibLinearTests.predictValues(model, dataset);
